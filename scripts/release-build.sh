...
Usage: scripts/release-build.sh [--check] [--output DIR] [--version X.Y.Z]
...
paths=(
  ...
  multi-cli scripts/install.sh scripts/uninstall.sh scripts/icon.icns
)
...
chmod +x "$STAGE_DIR/multi-cli" "$STAGE_DIR/scripts/install.sh" "$STAGE_DIR/scripts/uninstall.sh"
