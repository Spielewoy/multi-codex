...
По умолчанию учётные данные остаются внутри профиля, а поддерживаемые настройки, диалоги, агенты, навыки и плагины доступны совместно. Добавьте `--isolated`, если нужно отделить весь домашний каталог инструмента.
...
| Kiro | `kiro` | Windows, macOS, Linux | Пользователь ОС или изолированный домашний каталог |
| OpenCode | `opencode` | Windows, macOS, Linux | Изолированный домашний каталог инструмента |
| Windsurf | `windsurf` | Windows, macOS, Linux | Пользователь ОС или изолированный домашний каталог |
...
| `multi-cli new <tool>/<name> --isolated` | Создать профиль с изоляцией всего домашнего каталога; псевдонимы: `--isolate`, `-i` |
| `multi-cli new <tool>/<name> --from <template>` | Создать профиль из сохранённого шаблона |
...
| `multi-cli clone <tool>/<src> <tool>/<dest>` | Скопировать профиль |
...
| `multi-cli template save <tool>/<profile> <name>` | Сохранить шаблон без учётных данных |
| `multi-cli template list \| delete <name>` | Показать или удалить шаблоны |
| `multi-cli export <tool>/<name> [path]` | Экспортировать профиль |
| `multi-cli import <archive> <tool>/<name>` | Импортировать профиль |
...
Профили аккаунтов используют самую узкую безопасную границу. `--isolated` создаёт отдельный домашний каталог инструмента и ничего не разделяет. Продукты с фиксированной идентичностью учётных данных ОС используют пользователя Windows под управлением Multi-CLI и требуют терминал с повышенными правами.
